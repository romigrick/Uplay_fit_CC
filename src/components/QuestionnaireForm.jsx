        ...
        <div className="min-h-screen flex items-center justify-center py-20 px-4 bg-[#0C0C0C]">
            <div className="w-full max-w-xl bg-gray-900/50 border border-gray-800 rounded-[3rem] p-10 text-center">
                <AnimatePresence mode="wait">
                    ...
                    {step === 1 && (
                        <motion.div key="s0" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                            <h2 className="text-3xl font-black text-white mb-8 uppercase tracking-tighter">Para começar, <br/><span className="text-brand-gold">qual seu nome?</span></h2>
                            <div className="text-left mb-8">
                                ...
                                <Input 
                                    ...
                                    className="bg-white/5 border-white/10 text-white h-14 rounded-xl focus:border-brand-gold transition-all"
                                />
                            </div>
                        </motion.div>
                    )}
                    ...
                    {step === 2 && (
                        <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                            <h2 className="text-3xl font-black text-white mb-8 uppercase tracking-tighter">Qual seu <span className="text-brand-gold">Objetivo?</span></h2>
                            <div className="grid gap-4 mb-8">
                                {objectives.map((obj) => (
                                    <button
                                        key={obj.id}
                                        onClick={() => setFormData({...formData, objetivo: obj.label})}
                                        className={`p-6 rounded-2xl flex items-center justify-between border-2 transition-all ${formData.objetivo === obj.label ? 'border-brand-gold bg-brand-gold/10 text-white' : 'border-white/10 text-gray-400'}`}
                                    >
                    ...
                    {step === 3 && (
                        <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                            <h2 className="text-3xl font-black text-white mb-8 uppercase tracking-tighter">Quantas vezes <span className="text-brand-gold">na semana?</span></h2>
                            <div className="flex items-center justify-center gap-8 mb-10">
                                <input type="number" readOnly value={formData.frequencia} className="bg-transparent text-8xl font-black text-brand-gold w-32 text-center" />
                    ...
                    {step === 4 && (
                        <motion.div key="s3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                            <div className="bg-brand-gold/10 text-brand-gold p-4 rounded-2xl mb-6 inline-flex items-center gap-2 font-black text-xs uppercase tracking-widest">
                ...
                <div className="flex gap-4">
                    ...
                    <Button 
                        onClick={handleNext} 
                        className="flex-[2] py-7 rounded-2xl bg-brand-gold text-black font-black text-xl hover:bg-white hover:text-black hover:scale-[1.02] transition-all duration-300 shadow-[0_0_20px_rgba(177,248,42,0.2)]"
                    >
